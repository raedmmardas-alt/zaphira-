...
function mockAmazonFetch({ reportRows, reportStatusSequence } = {}) {
  ...
  const rows = reportRows ?? [
    { campaignId: 111111111111111, adGroupId: 333333333333333, advertisedAsin: 'B0EXAMPLE123', advertisedSku: 'COCO-16OZ', impressions: 300, clicks: 15, cost: 9.5, purchases1d: 2, sales1d: 39.98 },
  ];
  ...
  globalThis.fetch = async (url, opts) => {
    ...
    const isAmazonHost = s.includes('/auth/o2/token') || s.includes('/sp/campaigns/list') || s.includes('/sp/adGroups/list')
      || s.includes('/reporting/reports') || s === 'https://fake-s3.example.com/advertised-product-report.json.gz';
    ...
    if (s.endsWith('/reporting/reports')) return { ok: true, status: 200, text: async () => JSON.stringify({ reportId: 'ad-product-report-123', status: 'PENDING' }) };
    if (s.includes('/reporting/reports/ad-product-report-123')) {
      const status = statuses[Math.min(statusCallIndex, statuses.length - 1)];
      statusCallIndex++;
      if (status === 'COMPLETED') {
        return { ok: true, status: 200, text: async () => JSON.stringify({ status: 'COMPLETED', url: 'https://fake-s3.example.com/advertised-product-report.json.gz' }) };
      }
      return { ok: true, status: 200, text: async () => JSON.stringify({ status }) };
    }
    if (s === 'https://fake-s3.example.com/advertised-product-report.json.gz') {
...
async function postSync(payload) {
  const res = await fetch(`${baseUrl}/api/amazon/advertisedproducts/sync`, {
    method: 'POST',
...
async function getStatus() {
  return (await fetch(`${baseUrl}/api/amazon/advertisedproducts/status`)).json();
}

async function getResult() {
  return (await fetch(`${baseUrl}/api/amazon/advertisedproducts/result`)).json();
}
...
describe('POST /api/amazon/advertisedproducts/sync -- returns immediately, runs in the background', () => {
  ...
  test('normalizes advertised product rows, strengthening product mapping via the SAME AdvertisedProductRow model', async () => {
    mockAmazonFetch();
    ...
    assert.equal(result.rows[0].asin, 'B0EXAMPLE123');
    assert.equal(result.rows[0].sku, 'COCO-16OZ');
    assert.equal(result.rows[0].spend, 9.5);
  });
...
describe('POST /api/amazon/advertisedproducts/sync -- validation, failure, and concurrency', () => {
  ...
  test('a second sync request is rejected immediately while the first is still generating, and never triggers a second report', async () => {
    ...
    globalThis.fetch = async (url, opts) => {
      ...
      if (s.endsWith('/reporting/reports')) {
        reportRequestCount++;
        await gate;
        return { ok: true, status: 200, text: async () => JSON.stringify({ reportId: 'ad-product-report-123', status: 'PENDING' }) };
      }
      if (s.includes('/reporting/reports/ad-product-report-123')) {
        return { ok: true, status: 200, text: async () => JSON.stringify({ status: 'COMPLETED', url: 'https://fake-s3.example.com/advertised-product-report.json.gz' }) };
      }
...
describe('Amazon write operations remain unavailable on the advertised-product-sync router too', () => {
  test('no route exists for creating/updating/deleting product ads, campaigns, or bids under /api/amazon/advertisedproducts', async () => {
    const writeShapedRequests = [
      { method: 'POST', path: '/api/amazon/advertisedproducts' },
      { method: 'PUT', path: '/api/amazon/advertisedproducts/111' },
      { method: 'DELETE', path: '/api/amazon/advertisedproducts/111' },
    ];
